...
## Vite SPA (`web/`) — production frontend

`web/` is the production frontend (the Next.js `frontend/` was deleted in the Plan 3d cutover). Architecture: Vite + React 19, React Router 7 with `/:lang` (reusing the dictionary structure the Next app had), TanStack Query with `apiFetch` (`credentials: "include"`), `StaffGuard` probing `GET /api/v1/authentication/session` for staff-only routes. Tests: Vitest + RTL (`@testing-library/jest-dom/vitest`). Docker image: lightweight nginx serving `dist/` with `try_files` SPA fallback and `/api` proxy to the backend (`web/nginx.conf` hardcodes `backend:8080` — no `INTERNAL_API_URL`). Typecheck script: `pnpm typecheck` runs `tsc --noEmit`. Built TDD; `pnpm test`, `pnpm build`, and `pnpm typecheck` all pass. Local dev: `cd web && pnpm dev` (Vite dev server proxies `/api` → `http://localhost:8080`).
...
**Public pages ported (Plan 3b):** the 10 public routes are wired as client React Query pages under a `PublicLayout` (`SiteHeader` + `<main>`), each with `errorElement=NotFound`: home, `herbs`[/id], `remedies`[/id], `districts`[/id], `healers/[id]`, `search`, `treatment-cases`. Each ports its Next server component 1:1 — `useT`/`useParams`/`useSearchParams`, the server `Promise.all` body moves verbatim into a `queryFn`, `Skeleton` while pending, `<NotFound/>` on 404/bad id, and **every link is locale-prefixed** `/${lang}/…`. `SearchBox` was recreated as a client component (it used `next/root-params`); `SiteHeader`'s two links were locale-fixed. `DistrictPage` uses a direct `getDistrict`+`getProvince` read instead of the source's N-province fanout (`withinlazy`). Route code-splitting deferred (YAGNI; single 364 KB chunk, 111 KB gzip — revisit at 3d if large). `tsc` clean, 31 tests pass, `vite build` green. Staff routes + login remain for Plan 3c; `frontend/` still serves prod until 3d.

**Staff pages ported (Plan 3c):** login + 23 staff routes wired. `LoginPage` (public, under `PublicLayout`) inverts `StaffGuard` — a session probe redirects a logged-in user to `/${lang}/staff`. The 23 staff routes nest under `StaffGuard` (session probe → `/login` on 401) → `StaffLayout` (sidebar: 6 locale-prefixed `StaffNavLink`s with `match[]` also prefixed, + `LogoutButton`) → the pages: dashboard, provinces[/id]/edit + districts new/edit (district edit validates `provinceId` match), healers[/id]/edit + scoped remedies, remedies[/id]/edit + scoped cases, cases new/edit, herbs new/edit + a read-only `HerbUsagePage`. Staff list pages are `StaffRemediesPage`/`StaffHerbsPage` (the public `RemediesPage`/`HerbsPage` from 3b are untouched). Each page fetches parent option-lists via React Query (`listHealers`/`listRemedies` at `pageSize:48`, `getFirstProvince`+`listDistricts`) and renders the 3a list/form components; the 6 admin lists were locale-fixed (a 3a carry-over bug — bare `/staff` `<Link to>` dropped the `:lang` segment). `LoginForm`/`LogoutButton` `removeQueries(['session'])` before navigate so `StaffGuard`/`LoginPage` re-fetch fresh after a cookie change (no stale-401/stale-200 redirect bounce). Case pages drop an unused `listHealers` fetch (`healerId` comes from the remedy); remedy/case new-edit drop province/districts fetches the forms don't consume (`withinlazy`). `tsc` clean, 75 tests pass, `vite build` green.

**Cutover (Plan 3d):** `frontend/` (Next.js) is **deleted**; `web/` is the sole, production frontend. The root `compose.yaml` `frontend` service builds `./web` (`target: production`, same `:3000`, same service name); `compose.override.yaml`'s frontend dev override was dropped (`web/` dev runs on the host via `pnpm dev` — `web/Dockerfile` has no dev target); the prod template + root compose dropped the dead `INTERNAL_API_URL` env (nginx hardcodes `backend:8080`). CI's `frontend` job runs `typecheck`+`test`+`build` in `web/` (was `lint`+`test` in `frontend/`); `release.yml` builds the `thai-folk-medicine-frontend` GHCR image from `./web` (same image name, new context). Production now runs **Postgres + Go + nginx — no Node process**. `tsc` clean, 75 tests pass, `vite build` green, both compose configs + ansible render-check pass. **Release of a `v*` tag is the remaining gated step** (needs explicit sign-off — outward-facing). `withinlazy`: route code-splitting still deferred (single chunk) — revisit if the bundle grows.
...
- `deploy/`: Ansible playbook + `compose.prod.yaml.j2` (GHCR images) + `.env`
  ...
  reloads nginx. The prod compose ships a `seed` service (profile `seed`) for
  manual demo-data loading on the server; the deploy does not run it. See
