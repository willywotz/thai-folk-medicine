...
- when task done must update `CONTEXT.md` and commit it
- must strictly compliance with the 15-Factor App methodology
- must strictly compliance with Clean Architecture methodology
- must strictly compliance with event driven architecture methodology
- must strictly full english name not short or alias for api endpoint route 
...
- Mark deliberate simplifications that cut a real corner with a known ceiling (global lock, O(n²) scan, naive heuristic) with a `withinlazy:` comment naming the ceiling and upgrade path (`# withinlazy: global lock, per-account locks if throughput matters`).
...
- Follow style guides for all languages
  - [HTML/CSS](.claude/styles/google-html-css-style.md)
  - [JavaScript](.claude/styles/google-js-style.md)
  - [TypeScript](.claude/styles/google-ts-style.md)
  - [Python](.claude/styles/google-python-style.md)
  - [Go](.claude/styles/uber-go-style.md)
...
- Naming: Use American English
- Naming: Avoid plurals like "xxxList" for simplicity and to prevent singular/plural ambiguity stemming from poor design
